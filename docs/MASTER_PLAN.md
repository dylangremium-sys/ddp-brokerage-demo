...
**F-N5 — an unpriced batch stores `0`, not NULL. ~~Reads to a buyer as ฿0/kg.~~ CORRECTED 2026-08-07.** The storage half is right: `pricePerKg` is `parseFloat(x) || 0`, so "no price" is stored as zero. The audience was wrong. The buyer pack already guarded `> 0` and rendered "—"; it was the **farmer's own** stock list and status page that rendered `฿0/kg`, telling a farm its crop was priced at zero. Fixed by routing all five price renderings through one formatter, which also fixed a defect this finding missed entirely: **every one of them hardcoded the baht sign**, so a USD batch displayed as baht on every screen. Recorded as a lesson — a finding written from one surface will describe that surface's behaviour as though it were the system's.

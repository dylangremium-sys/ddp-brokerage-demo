...
4. ~~**[VERIFIED] The remediation-sprint work is uncommitted.**~~ **[CORRECTED — no longer true.]** All Phase-D changes are committed, pushed and **merged**: the guard fix and staging-suite hardening via **PR #3** (`4a77828`), and the decision store, snapshot store and migration 17 via **PR #4** (`e8a93c7`). **PR #6** (`febfd99`) then corrected the migration status wording. Current `main` = **`5b21999`**; these reports are **PR #5** (open). **Migrations: STAGING has 10 and 17 applied and verified (2026-07-14); PRODUCTION has neither, still uses the fallback, and has had no SQL verification.**
...
| — | ~~Remediation code uncommitted~~ **[CORRECTED — CLOSED]** | **RESOLVED** | The code is merged to `main` (`5b21999`): PR #3 (`4a77828`), PR #4 (`e8a93c7`) and PR #6 (`febfd99`). PR #5 (these docs) remains open. The stated risk ("one `git checkout` from loss") no longer exists. |
...
| **Developer workflow** | **58** | [VERIFIED] Strong static CI (**523** tests on `main` today, measured; 464 when written — plus tsc, lint, build) + live staging security suite. Weak: no CD, migrations by hand. **[CORRECTED]** "reviewed work uncommitted" no longer applies — the work is merged. |
| **Testing** | **56** | [VERIFIED] **523** unit tests (pure logic) on `main` today, measured; 464 when written — plus live staging security suite. Zero component/E2E tests; zero AI evals. |
...
**Immediate (this week)**
- ~~Commit the verified Phase-D working-tree changes to a branch and open a PR~~ **[CORRECTED — DONE]** PR #3 (`4a77828`), PR #4 (`e8a93c7`) and PR #6 (`febfd99`) are **merged**; PR #5 remains open. *Value: the reviewed code becomes reproducible/deployable.*

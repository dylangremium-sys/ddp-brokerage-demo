...
>
> This falsification pass describes **`f5c8cbb`**. It was committed (`9ccffab`) alongside the remediation (`94c5c42`), which was later split: **PR #3 (`4a77828`) and PR #4 (`e8a93c7`) are now both MERGED** to `main` (current `main` = `5b21999`); **PR #5** is these documents. Left as written, as the historical record. Against the current tree — noting throughout that **code merged ≠ production database migrated**:
>
> - **Finding 2 — "The immutable snapshot RPC has never been called … Zero invocations. No wrapper, no indirect call, no server-side use, no client-side use. VERIFIED FACT."** (`:60`, `:78`, `:112`). **SUPERSEDED — now FALSE of `main`.** PR #4 is merged (`e8a93c7`): `main` contains the caller (`src/lib/buyerPackSnapshotSupabaseStore.ts`, wired at `DDPBuyerPreview.tsx`), and the RPC was exercised end-to-end in STAGING inside a rolled-back transaction. The "Built ✅ / Connected ❌" row is now **Built ✅ / Connected ✅ in code**. It remains **not active in PRODUCTION**, where migration 10 is unapplied and the app degrades to its fallback.
...
> - **Finding 1** (localStorage as the release-layer system of record) is **addressed in code and in STAGING, but NOT in production.** The server-authoritative, append-only path (`procurementDecisionStore.ts`, migration 17) is **merged to `main`** (PR #4, `e8a93c7`), and migration 17 is **applied and verified in staging**. In **PRODUCTION** the migration is **not applied**, so the release-layer decision there still lives in browser localStorage: **as a statement about production runtime, this finding still stands.**

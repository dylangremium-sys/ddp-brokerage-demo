...
**The dominant risk is no longer missing code — it is unverified *production* database state.** The repository's own runtime ledger (`docs/MIGRATION_RUNTIME_STATUS.md`, last verified 2026-07-21) is the current authority for runtime application status and now covers migrations 10, 17 and **19–23** — the farm admin-field guard, controlled farmer provisioning, the operational-farmer RLS overlay, and server-authoritative Buyer Pack issuance. **Staging is largely closed:** 20, 21 and 22 are `APPLIED_AND_VERIFIED`; 19 and 23 are `APPLIED_NOT_VERIFIED` (installed, behaviour not fully exercised) — see *Migrations 19–23 — status matrix*. Migration 23 in particular **is** installed in staging, superseding its runbook's earlier "runs no SQL against any database" claim; see *Conflicting evidence — migration 23 in staging*. **Production remains `UNKNOWN` for all five** — it was not contacted. So the platform's security posture is now *confirmed live on staging* but still *unconfirmed on production*, and that production gap is the single most important remaining item in the programme.
...
**Runtime verification status:** `docs/MIGRATION_RUNTIME_STATUS.md` now records migrations **21 and 22 as `APPLIED_AND_VERIFIED` in staging** — see *Migrations 19–23 — status matrix*. Migration 21's own VERIFY script defers the "non-admin JWT cannot self-promote" proof to the live staging harness (`21_..._VERIFY.sql:14-16`), and that proof was obtained: the 2026-07-21 run recorded farmers A and B each denied self-elevation by RLS (`SQLSTATE 42501`) with role unchanged. Staging enforcement is therefore **confirmed live**, not merely authored. **Production remains `UNKNOWN`** — it was not contacted, and no production verification has been performed.
...
1. Migrations **19, 20, 21, 22, 23 are now recorded** in `docs/MIGRATION_RUNTIME_STATUS.md`, which is the current authority for runtime application status. **Staging:** 20, 21 and 22 are `APPLIED_AND_VERIFIED`; 19 and 23 are `APPLIED_NOT_VERIFIED`. **Production: `UNKNOWN` for all five** — production was not contacted.
...
2. **DONE (2026-07-21).** `docs/MIGRATION_RUNTIME_STATUS.md` now covers migrations **19 through 23** with per-environment status and dates, alongside 10 and 17. Staging: 20/21/22 `APPLIED_AND_VERIFIED`, 19/23 `APPLIED_NOT_VERIFIED`. Production: `UNKNOWN` for all five. See *Migrations 19–23 — status matrix*.

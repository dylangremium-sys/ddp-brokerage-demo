...
- **Status** — **PLANNED (substrate live, product absent).** Corrected 2026-08-06 against production; the previous wording said no buyer role or table existed anywhere in the schema, and that is false. Production's `profiles_role_check` admits `'buyer'`, and `organisations` and `organisation_memberships` both exist (migration 39), holding **0 rows** with **0 references** anywhere in `src/` or `api/`. What is absent is the *product*: `UserRole` (`src/services/auth.ts:16`) omits `buyer`, so `resolvePostLoginDecision` falls a buyer account through to `default:` and signs it straight back out; there is no buyer page in the `Page` union; and none of the 5 policies on `inventory_batches` is buyer-facing. "Buyer Preview" remains DDP staff previewing what a buyer *would* see. Do not restate this section as "no buyer role exists" — an audit did, from this line, and reached a false conclusion. See `docs/MASTER_PLAN.md` §3.2 and W3.

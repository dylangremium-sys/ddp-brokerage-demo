...
## Current decision: 🟢 GO for controlled pilot — one recommended live confirmation (SG-6 UI) remaining

**Why (updated 2026-07-24, staging pass #4 — gates closed):** SG-1, SG-2, SG-3, SG-4, SG-5 all **PASS with live staging evidence**; SG-6 is code + server-authoritative-verified. The SG-3 gap is fixed — the `farmer-documents` bucket now enforces `application/pdf` + 10 MB server-side (non-PDF upload → HTTP 400). The security suite was updated to upload PDF-typed probes (so it exercises RLS, not the mime gate) and is green again (111/0/0). **Only residual:** SG-6 has not been clicked through in a live browser this session — the enforcement is proven at code + DB level (migration 23 server-authoritative, print-CSS fail-closed), so residual risk is low; a single live export click-through is recommended as final confirmation before the first real farm.
...
| SG-3 Upload smoke (happy+fail) | ✅ PASS | Bucket `farmer-documents` now `file_size_limit=10485760`, `allowed_mime_types={application/pdf}` (applied 2026-07-24, = migration 8 intent). Live: Farmer-A **PDF upload HTTP 200**, signed-URL retrieval HTTP 200, **non-PDF upload rejected HTTP 400**, RLS confines to own prefix. Security suite storage probes updated to PDF-typed uploads so they exercise RLS not the mime gate → suite green 111/0/0 |

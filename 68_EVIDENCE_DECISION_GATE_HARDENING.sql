-- 68_EVIDENCE_DECISION_GATE_HARDENING.sql
--
-- WHAT THIS CLOSES.
--
...
BEGIN
  IF to_regclass('public.schema_migrations') IS NULL THEN
    RAISE EXCEPTION
      'Apply 67 (the migrations ledger) before this migration. Without it there is no record of what landed, which is the failure 62 exists to prevent.'
      USING ERRCODE = 'check_violation';
  END IF;

  INSERT INTO public.schema_migrations (number, name, applied_at, applied_by, evidence)
  VALUES (68, 'EVIDENCE_DECISION_GATE', now(), current_user, 'self-recorded')

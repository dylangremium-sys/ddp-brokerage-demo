...
function buildDeps(): CoaExtractionDeps | null {
  ...
  return {
    ...
    getDocument: async (documentId) => {
      // Query inventory_batches (the real COA location) using the batch ID.
      // COA file info is stored in: coa_file_name, coa_storage_path, coa_sha256_hex
      const { data, error } = await admin
        .from('inventory_batches')
        .select('id, coa_file_name, coa_storage_path, coa_sha256_hex')
        .eq('id', documentId)
        .maybeSingle()
      if (error || !data) return null
      if (!data.coa_file_name || !data.coa_storage_path) return null
      return {
        id: data.id,
        fileName: data.coa_file_name,
        storagePath: data.coa_storage_path,
        sha256Hex: data.coa_sha256_hex,
      }
    },
    ...
    persistExtractions: async (documentId, rows, recordedByUserId) => {
      if (!rows || rows.length === 0) {
        throw new Error('coa_extract_no_rows_to_persist')
      }
      
      // Map from extraction rows to database insert format
      const dbRows = rows.map(({ reportNumber, row }) => ({

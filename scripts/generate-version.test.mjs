...
describe('the version stamp survives a build container with no git', () => {
  it('the sandbox genuinely has no git, or every case below is vacuous', () => {
    ...
    const result = spawnSync('git', ['rev-parse', 'HEAD'], { cwd: sandbox, encoding: 'utf8' })
    expect(result.status).not.toBe(0)
  })

  it('falls back to VERCEL_GIT_COMMIT_SHA, which Vercel sets for CLI deploys from a repo', () => {
    const sha = 'a'.repeat(40)
    const result = runIn(sandbox, { VERCEL_GIT_COMMIT_SHA: sha })
    expect(result.status).toBe(0)
    expect(result.json.commitSha).toBe(sha)
    expect(result.json.commitShaShort).toBe(sha.slice(0, 7))
  })

  it('falls back to DDP_COMMIT_SHA, the explicit escape hatch', () => {
    const sha = 'b'.repeat(40)
    const result = runIn(sandbox, { DDP_COMMIT_SHA: sha })
    expect(result.status).toBe(0)
    expect(result.json.commitSha).toBe(sha)
  })

  it('prefers VERCEL_GIT_COMMIT_SHA over DDP_COMMIT_SHA when both are set', () => {
    const result = runIn(sandbox, { VERCEL_GIT_COMMIT_SHA: 'c'.repeat(40), DDP_COMMIT_SHA: 'd'.repeat(40) })
    expect(result.json.commitSha).toBe('c'.repeat(40))
  })

  it('with no git and no env, still exits 0 but WARNS — it must never fail a build', () => {
    const result = runIn(sandbox, {})
    expect(result.status).toBe(0)
    expect(result.json.commitSha).toBe('unknown')
    expect(result.stderr).toMatch(/will not be able to say which commit/i)
  })
  ...
  it('an empty env var is treated as absent, not as a commit', () => {
    ...
    const result = runIn(sandbox, { VERCEL_GIT_COMMIT_SHA: '   ' })
    expect(result.json.commitSha).toBe('unknown')
  })
})
...
describe('git remains authoritative when it is available', () => {
  it('a sandbox WITH a git repo stamps the real HEAD, ignoring the env var', () => {
    ...
    try {
      ...
      const result = runIn(dir, { VERCEL_GIT_COMMIT_SHA: 'e'.repeat(40) })
      expect(result.json.commitSha).toBe(head)
    } finally {

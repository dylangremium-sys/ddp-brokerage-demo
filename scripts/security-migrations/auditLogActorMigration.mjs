...
export function findVerifyProblems(sql) {
  ...
  if (!/insert\s+into\s+public\.compliance_audit_log/i.test(ver)) vp.push('VERIFY has no behavioural INSERT (vacuous)')
  if (!/raise\s+exception/i.test(ver)) vp.push('VERIFY has no RAISE assertions (vacuous)')
  if (!/request\.jwt\.claims/i.test(ver)) vp.push('VERIFY does not impersonate a caller via the request JWT')
  if (!/victim/i.test(ver)) vp.push('VERIFY does not exercise a forged (different) actor_id')
  if (!/leftover|residue/i.test(residueTail)) vp.push('VERIFY has no post-rollback residue check')
  return vp
}
...
export function findRollbackProblems(sql) {
  ...
  const rp = []
  if (!/drop\s+trigger\s+if\s+exists\s+compliance_audit_log_set_actor\s+on\s+public\.compliance_audit_log/i.test(rb)) rp.push('does not drop the trigger')
  if (!/drop\s+function\s+if\s+exists\s+public\.fn_compliance_audit_log_set_actor/i.test(rb)) rp.push('does not drop the function')
  if (/drop\s+(table|policy)\b|compliance_audit_log_no_update_delete/i.test(rb)) rp.push('drops the table/policy/append-only trigger (overreach)')
  return rp
}

# DDP Brokerage Demo Script — 5-Minute Walkthrough

**Live URL:** https://ddp-brokerage-demo.onrender.com  
**Audience:** Buyers, investors, or new operator partners  
**Duration:** 4–6 minutes  
**Prerequisite:** Demo accounts provisioned (see HANDOVER_CHECKLIST.md)

---

## 1. Landing Page (~30 seconds)

Open the live URL. The landing page loads with two entry points:

- **"Enter Farmer Portal"** — for cannabis farm operators registering supply
- **"Enter DDP Operations"** — for DDP's internal brokerage and compliance team

Talking points:
> "DDP sits between compliant Thai cannabis farms and international buyers.
> Farmers submit their profiles, COA documents, and stock through their portal.
> DDP's operations team reviews, approves, and packages verified supply for buyers."

---

## 2. Farmer Portal — Signup & Dashboard (~60 seconds)

Click **Enter Farmer Portal**. This routes to the signup form.

Point out the single-card layout: brand header, divider, then the form.
The province selector shows all 77 Thai provinces.
Roles available: Farmer, Farm Manager, Broker.

> "Farmers sign up in under two minutes — just a phone number, province,
> and password. No email required. LINE ID is optional for follow-up."

For the demo, click **Already have an account? Sign in** and log in with the farmer demo account.

After login, the **Farmer Dashboard** shows:
- Welcome card with profile completion percentage
- Quick-action grid: Build Profile / My Stock / My Activity / Requests / Advanced Profile
- Profile completion bar and missing-field hints

> "The dashboard shows the farmer's most recently created profile and
> tracks how complete their submission is — licence, COA, export docs."

---

## 3. Farmer — Build Profile (~60 seconds)

Click **Build Profile** (or the nav link).

Show the multi-step farm profile form:
- Business info (legal name, trading name, province, GPS)
- Ownership & licences (cultivation licence, export licence)
- Facility & cultivation details
- Lab testing / COA upload
- Export readiness

> "DDP scores each farm on a 0–100 basis across these categories.
> Farms need at least 60% to be eligible for buyer introductions."

---

## 4. Farmer — My Stock (~30 seconds)

Click **My Stock** in the nav.

Show the inventory batch list (or empty state if no data). Point out:
- Product type (flower, extract, biomass)
- Quantity in kg
- THC/CBD % fields
- Batch status (Pending Review / Approved / Missing Document / Rejected)

> "Each harvest batch is submitted separately. DDP reviews each one
> before it appears to buyers."

---

## 5. Admin Operations — Overview (~60 seconds)

Sign out (or open a new private window). Click **Enter DDP Operations** and log in with the admin demo account.

The **Operations Overview** shows:
- Total farms onboarded / pending / approved / watchlist
- Total kg in system / approved kg
- Items with missing documents
- Export-ready farm count
- Recent farm submissions and recent inventory batches

> "This is DDP's internal view. The team sees everything across all farms
> in real time — who's submitted, who's approved, and what's export-ready."

---

## 6. Admin — Farm Profiles (~30 seconds)

Click **Farm Profiles** in the admin nav.

Show the farm list with DDP compliance scores, status badges, and the ability to open a farm for review. The review panel lets DDP change farm status, add notes, and score the submission.

> "DDP's analyst reviews the profile, checks the cultivation licence number,
> verifies the COA, and updates the status — which the farmer sees immediately."

---

## 7. Admin — Inventory Review (~30 seconds)

Click **Inventory Review**.

Show pending and approved batches. Click a batch to open the review panel. DDP can approve, flag missing documents, or reject with a note.

> "Every batch goes through this queue. We only pass verified, documented
> supply to buyers — no unverified product touches the buyer view."

---

## 8. Admin — Master Inventory (~30 seconds)

Click **Master Inventory**.

Show the consolidated view of all approved supply: product type, quantity, THC/CBD, farm name, batch number. This is the source of truth for what DDP can offer.

> "This is DDP's live catalogue — all approved, compliant, verified supply.
> When a buyer asks 'what do you have?', this is the answer."

---

## 9. Admin — Buyer Preview (~60 seconds)

Click **Buyer Preview**.

Show the buyer-facing package for a specific inventory batch:
- Farm summary (name, province, partner tier)
- Compliance scorecard (licence, COA, export readiness, etc.)
- Lab values (THC/CBD/terpenes)
- DDP Verified Supply seal
- **Print / Save PDF** button — generates a PDF buyer pack
- **Copy Summary** button — copies a plain-text version for email/WhatsApp

> "This is what a buyer receives from DDP — a verified product dossier.
> The farmer never deals with the buyer directly. DDP handles all introductions,
> NDA compliance, and contract facilitation."

---

## Closing (15 seconds)

> "DDP is the verified supply chain layer that Thai cannabis farms need
> to access international markets. We handle compliance, documentation,
> buyer matching, and contract facilitation — so farmers focus on growing
> and buyers get verified supply they can trust."

---

## Demo Reset (if needed)

If you need to reset to a clean state between demos, contact the DDP tech team.
Do not use the RESET SQL files without a full backup and explicit approval.

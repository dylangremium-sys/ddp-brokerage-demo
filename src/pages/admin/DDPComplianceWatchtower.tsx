...
export default function DDPComplianceWatchtower({ farms, inventory, currentUser }: Props) {
  ...
  const [legalUpdates, setLegalUpdates] = useState<LegalUpdate[]>(() => loadStored(STORAGE.legalUpdates, []))
  const [reviews, setReviews] = useState<ComplianceReview[]>(() => loadStored(STORAGE.reviews, []))
  const [rules, setRules] = useState<ComplianceRule[]>(() => loadStored(STORAGE.rules, createBaselineComplianceRules()))
  const [storedAlerts, setStoredAlerts] = useState<ComplianceAlert[]>(() => loadStored(STORAGE.alerts, []))
  const [auditLog, setAuditLog] = useState<ComplianceAuditLog[]>(() => loadStored(STORAGE.audit, []))
  ...
  function writeAudit(entry: Omit<ComplianceAuditLog, 'id' | 'actorType' | 'actorId' | 'actorName' | 'createdAt'>): void {
  ...
  function persistLegalUpdates(next: LegalUpdate[]): void {
    setLegalUpdates(next)
    saveStored(STORAGE.legalUpdates, next)
  }

  function persistReviews(next: ComplianceReview[]): void {
    setReviews(next)
    saveStored(STORAGE.reviews, next)
  }

  function persistRules(next: ComplianceRule[]): void {
    setRules(next)
    saveStored(STORAGE.rules, next)
  }

  function persistAlerts(next: ComplianceAlert[]): void {
  ...
  function submitLegalUpdate(): void {
    ...
    persistLegalUpdates([update, ...legalUpdates])
  ...
  function updateReviewDecision(review: ComplianceReview, decision: string): void {
    const now = new Date().toISOString()
    ...
    persistReviews(reviews.map(item => item.id === review.id ? nextReview : item))
    persistLegalUpdates(nextLegalUpdates)
  ...
  function updateRuleStatus(rule: ComplianceRule, status: ComplianceRule['status']): void {
    const now = new Date().toISOString()
    ...
    persistRules(rules.map(item => item.id === rule.id ? updated : item))
  ...
  function submitManualAlert(): void {
    ...
    const alert: ComplianceAlert = {
      ...
      status: alertForm.severity === 'critical' ? 'blocked' : 'open',
      createdAt: now,
      resolvedAt: null,
      resolutionNotes: null,
    }
    persistAlerts([alert, ...storedAlerts])
    writeAudit({
  ...
  function updateAlertStatus(alert: ComplianceAlert, status: ComplianceStatus): void {
    const now = new Date().toISOString()
    ...
    persistAlerts(existing ? storedAlerts.map(item => item.id === alert.id ? updated : item) : [updated, ...storedAlerts])
    writeAudit({
    ...
    <div className="page-wrap ddp-wrap">
      ...
      {tab === 'monitor' && (
        <div className="card" style={{ padding: 22, marginTop: 16 }}>
          ...
          <button className="btn btn-primary" style={{ marginTop: 18 }} onClick={submitLegalUpdate}>Create Legal Update + Review Item</button>
        </div>
      )}
                    ...
                    <tr key={review.id}>
                      ...
                      <td style={{ minWidth: 250 }}>
                        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                          <button className="btn btn-review" onClick={() => updateReviewDecision(review, 'informational')}>Mark informational</button>
                          <button className="btn btn-review" onClick={() => updateReviewDecision(review, 'create_rule')}>Create suggested rule</button>
                          <button className="btn btn-review" onClick={() => updateReviewDecision(review, 'approve_rule')}>Approve rule</button>
                          <button className="btn btn-review" onClick={() => updateReviewDecision(review, 'send_to_legal')}>Send to legal</button>
                          <button className="btn btn-review" onClick={() => updateReviewDecision(review, 'reject')}>Reject</button>
                          <button className="btn btn-review" onClick={() => updateReviewDecision(review, 'archive')}>Archive</button>
                        </div>
                      </td>
                    </tr>
                  )
                })}
                {reviews.length === 0 && <tr><td colSpan={7} style={{ textAlign: 'center', padding: 28 }}>No review items yet.</td></tr>}
              </tbody>
            </table>
          </div>
        </div>
              ...
              <tbody>
                {rules.map(rule => (
                  <tr key={rule.id}>
                    ...
                    <td>{rule.approvedBy || '—'}<br /><span className="td-muted">{rule.approvedAt || 'Pending human approval'}</span></td>
                    <td>
                      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                        <button className="btn btn-review" onClick={() => updateRuleStatus(rule, 'active')}>Approve / Activate</button>
                        <button className="btn btn-review" onClick={() => updateRuleStatus(rule, 'paused')}>Pause</button>
                        <button className="btn btn-review" onClick={() => updateRuleStatus(rule, 'retired')}>Retire</button>
                        <button className="btn btn-review" onClick={() => updateRuleStatus(rule, 'rejected')}>Reject</button>
                      </div>
                    </td>
                  </tr>
                ))}
              ...
              <tbody>
                {readiness.map(record => (
                  <tr key={record.id}>
                    ...
                    <td><button className="btn btn-review" onClick={() => setOpenReadinessId(openReadinessId === record.id ? null : record.id)}>{openReadinessId === record.id ? 'Close' : 'Open checklist'}</button></td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      {tab === 'alerts' && (
        <>
          <div className="card" style={{ padding: 20, marginTop: 16 }}>
            ...
            <button className="btn btn-primary" style={{ marginTop: 16 }} onClick={submitManualAlert}>Create Alert</button>
          </div>
          <div className="card table-card" style={{ marginTop: 16 }}>
            <div className="table-scroll">
              <table className="inv-table inv-table--cards">
                  ...
                  {alerts.map(alert => (
                    <tr key={alert.id}>
                      ...
                      <td><div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                        <button className="btn btn-review" onClick={() => updateAlertStatus(alert, 'in_review')}>In review</button>
                        <button className="btn btn-review" onClick={() => updateAlertStatus(alert, 'blocked')}>Block</button>
                        <button className="btn btn-review" onClick={() => updateAlertStatus(alert, 'resolved')}>Resolve</button>
                        <button className="btn btn-review" onClick={() => updateAlertStatus(alert, 'dismissed')}>Dismiss</button>
                      </div></td>
                    </tr>
                  ))}
          ...
          <div className="table-scroll">
            <table className="inv-table inv-table--cards">
              ...
              <tbody>
                ...
                {auditLog.length === 0 && <tr><td colSpan={5} style={{ textAlign: 'center', padding: 28 }}>No audit events yet.</td></tr>}
              </tbody>
            </table>
          </div>
        </div>

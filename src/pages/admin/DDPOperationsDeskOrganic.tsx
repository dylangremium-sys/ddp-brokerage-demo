      ...
      <header style={{ display: 'flex', justifyContent: 'space-between', gap: 32, alignItems: 'end', flexWrap: 'wrap' }}>
        ...
        <div style={{ display: 'flex', gap: 10, alignItems: 'center', flexWrap: 'wrap' }}>
          ...
          <button
            ...
            disabled={chases.length === 0 || !onChaseFarms || busy}
            style={chases.length === 0 || !onChaseFarms || busy
              ? { opacity: 0.45, cursor: 'not-allowed' }
              : undefined}
            onClick={() => { onChaseFarms?.(chases); setSelected(new Set()) }}
          >
            {busy
              ? 'Recording…'
              : chases.length === 0
                ? 'Chase farms'
                : `Chase ${chases.length} farm${chases.length === 1 ? '' : 's'}`}
          </button>
        </div>
      </header>
      ...
      <div style={{
        ...
        {visible.length === 0 && (
          <p style={{ padding: '28px 26px', margin: 0, fontSize: 14.5, color: 'var(--color-neutral-700)' }}>
            {emptyState === 'failed'
              ? 'Some matters could not be listed — a source failed to load, so this is not an all-clear.'
              : emptyState === 'loading'
                ? 'Still loading. This is not yet an all-clear.'
          ...
          return (
            ...
            >
              ...
              <input
                ...
                disabled={farmIdFor(item) === null}
                title={farmIdFor(item) === null
                ...
                style={{
                  width: 18, height: 18,
                  cursor: farmIdFor(item) === null ? 'not-allowed' : 'pointer',
                  opacity: farmIdFor(item) === null ? 0.4 : 1,
                }}
              />

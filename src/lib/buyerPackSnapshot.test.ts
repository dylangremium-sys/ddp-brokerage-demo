...
describe('createBuyerPackSnapshot — approval gating', () => {
  ...
  it.each([
    ['packId', { packId: '   ' }, /pack id/i],
    ['approvalId', { approvalId: '   ' }, /approval id/i],
    ['approvalTimestamp', { approvalTimestamp: '   ' }, /approval timestamp/i],
    ['generatedBy', { generatedBy: '   ' }, /generated by/i],
  ])('rejects creation when %s is blank', async (_name, overrides, matcher) => {
    await expect(createBuyerPackSnapshot(makeInput(overrides))).rejects.toThrow(matcher)
  })

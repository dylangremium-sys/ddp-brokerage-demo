...
describe('feed media types', () => {
  const FEED_XML = '<?xml version="1.0"?><rss version="2.0"><channel><title>SÚKL</title></channel></rss>'

  // Measured against the live feeds on 2026-08-02: sukl.gov.cz/feed/ answers
  // `application/rss+xml; charset=UTF-8` and EUR-Lex answers `application/xml`.
  // Those are the only two feed-modality sources in the registry, so a policy
  // that admits one and not the other silently halves regulatory monitoring.
  it.each([
    ['application/rss+xml; charset=UTF-8'],
    ['application/atom+xml'],
    ['application/xml'],
    ['text/xml'],
  ])('accepts a feed served as %s', async contentType => {
    const result = await retrieveOfficialSource({
      url: BASE_URL,
      policy,
      retrievedAt: RETRIEVED_AT,
      fetchImpl: fetchReturning(makeResponse({ contentType, body: FEED_XML })),
    })
    expect(result.status).toBe('retrieved')
  })

  it('returns feed XML verbatim rather than running it through htmlToText', async () => {
    const result = await retrieveOfficialSource({
      url: BASE_URL,
      policy,
      retrievedAt: RETRIEVED_AT,
      fetchImpl: fetchReturning(makeResponse({ contentType: 'application/rss+xml', body: FEED_XML })),
    })
    // The downstream RSS parser needs real markup. If the HTML text-stripper
    // ever starts applying to feeds, the tags vanish and every feed parses as
    // zero items — a failure that looks like "the regulator published nothing".
    expect(result.content).toBe(FEED_XML)
    expect(result.content).toContain('<channel>')
  })

  it('offers the feed types in Accept, so a server cannot negotiate away from the feed', async () => {
    const seen: SourceFetchInit[] = []
    const spyFetch: SourceFetchImpl = async (_url, init) => {
      seen.push(init)
      return makeResponse({ contentType: 'application/rss+xml', body: FEED_XML })
    }
    await retrieveOfficialSource({ url: BASE_URL, policy, retrievedAt: RETRIEVED_AT, fetchImpl: spyFetch })
    expect(seen[0].headers.Accept).toContain('application/rss+xml')
    expect(seen[0].headers.Accept).toContain('application/atom+xml')
  })

  it('still rejects a non-document type', async () => {
    const result = await retrieveOfficialSource({
      url: BASE_URL,
      policy,
      retrievedAt: RETRIEVED_AT,

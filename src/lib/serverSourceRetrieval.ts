...
export const DEFAULT_RETRIEVAL_POLICY: Required<Omit<SourceRetrievalPolicy, 'allowedHosts' | 'allowedPorts'>> = {
  ...
  // `application/rss+xml` and `application/atom+xml` are the media types real
  // feeds actually serve, and they are NOT interchangeable with
  // `application/xml`: measured 2026-08-02, `sukl.gov.cz/feed/` returns
  // `application/rss+xml; charset=UTF-8` while EUR-Lex returns
  // `application/xml`. Without both entries the SSRF-guarded retriever rejects
  // one of the only two working feeds at the content-type gate — and does so as
  // `rejected_content_type`, which reads like a hostile response rather than a
  // policy that is simply too narrow. This list is also sent verbatim as the
  // `Accept` header, so omitting them additionally invites a server to
  // content-negotiate away from the feed.
  allowedContentTypes: [
    'text/html',
    'text/plain',

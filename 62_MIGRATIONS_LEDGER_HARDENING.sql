-- 62_MIGRATIONS_LEDGER_HARDENING.sql
--
-- WHAT THIS CLOSES.
--
-- On 2026-08-11 migration 66 was reported applied to production three times.
...
-- today failed because it depended on the person who had just done the applying.
-- This table holds migration numbers and names — nothing sensitive — so it is
...
-- at, and absence here means "unknown", not "not applied". Forward from 62,
...
INSERT INTO public.schema_migrations (number, name, evidence)
SELECT 66, 'EVIDENCE_DECISION_GATE',
...
VALUES (62, 'MIGRATIONS_LEDGER', now(), current_user, 'self-recorded')
ON CONFLICT (number) DO NOTHING;

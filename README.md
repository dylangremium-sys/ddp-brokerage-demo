...
- **Reference guard** (`aiSourceReferenceGuard.ts`) — every entry the model returns under "Source references" must be the recorded source name/URL/title, or a verbatim quotation from the recorded raw evidence. Anything else is discarded before a reviewer sees it, and the discarded count is carried to the browser and shown. What is *displayed* is always read back out of the record — the enclosing sentence for a quotation, the recorded value for a metadata match — never the model's own string, so a matched span cannot drop a negation and cite the source as saying the opposite of what it says.
...
| Buyer Pack snapshots | `10_BUYER_PACK_SNAPSHOTS_MVP.sql` | `10_..._VERIFY.sql`, `10_..._ROLLBACK.sql` | Implemented | **Staging: applied + verified 2026-07-14. Production: APPLIED_AND_VERIFIED** (2026-07-26, `docs/MIGRATION_RUNTIME_STATUS.md`) |
...
| Procurement decisions | `17_PROCUREMENT_DECISIONS_MVP.sql` | `17_..._VERIFY.sql`, `17_..._ROLLBACK.sql` | Implemented | **Staging: applied + verified 2026-07-14. Production: APPLIED_AND_VERIFIED** (2026-07-26, `docs/MIGRATION_RUNTIME_STATUS.md`) |
...
| Farm admin-field guard | `19_FARM_ADMIN_FIELD_GUARD_HARDENING.sql` | `19_..._VERIFY.sql`, `19_..._ROLLBACK.sql` | Implemented; supersedes the earlier draft guard | **Staging: `APPLIED_NOT_VERIFIED`** (installed; behavioural coverage partial — UPDATE path only, 4 of 7 protected columns). **Production: APPLIED_AND_VERIFIED** (2026-07-26, `docs/MIGRATION_RUNTIME_STATUS.md`) |
| Guard ACL correction | `20_FARM_ADMIN_FIELD_GUARD_ACL_FIX.sql` | **no rollback script exists** | Implemented | **Staging: `APPLIED_AND_VERIFIED`. Production: APPLIED_AND_VERIFIED** (2026-07-26, `docs/MIGRATION_RUNTIME_STATUS.md`). Makes the manual Production `REVOKE` durable for fresh environments. |
| Controlled farmer provisioning | `21_DDP_CONTROLLED_FARMER_PROVISIONING_HARDENING.sql` | `21_..._VERIFY.sql`, `21_..._ROLLBACK.sql` | Implemented | **Staging: `APPLIED_AND_VERIFIED`. Production: APPLIED_NOT_VERIFIED** (2026-07-26, catalog present; read-only role lacks schema `auth` USAGE, so behavioural verification incomplete; `docs/MIGRATION_RUNTIME_STATUS.md`) |
| Operational-farmer access overlay | `22_OPERATIONAL_FARMER_ACCESS_RLS_HARDENING.sql` | `22_..._VERIFY.sql`, `22_..._ROLLBACK.sql` | Implemented | **Staging: `APPLIED_NOT_VERIFIED`** — installed, and the 11-table overlay is substantially verified, but the storage half of the policy (`FOR ALL`) has incomplete behavioural coverage: no pending-user storage UPDATE or DELETE enforcement probe, and the list probe failed. **Production: PARTIALLY_APPLIED** (2026-07-26, 11-table overlay present; storage policy missing per `docs/MIGRATION_RUNTIME_STATUS.md`) — security gap documented in PR #63 for post-pilot remediation |
| Server-authoritative Buyer Pack issuance | `23_BUYER_PACK_SERVER_AUTHORITATIVE_ISSUANCE.sql` | `23_..._VERIFY.sql`, `23_..._ROLLBACK.sql` | Implemented | **Staging: `APPLIED_NOT_VERIFIED`** — 2026-07-21 catalog evidence shows `issue_buyer_pack_snapshot` present and reading `procurement_decisions_current` (the migration-23 definition). Behavioural issuance verification not completed. **Production: APPLIED_AND_VERIFIED** (2026-07-26, Section A passed; live body reads `procurement_decisions_current`, confirming the migration-23 hardened version is installed, not migration 10's client-trusting variant; `docs/MIGRATION_RUNTIME_STATUS.md`) |

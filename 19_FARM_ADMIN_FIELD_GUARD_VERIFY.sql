...
begin
  ...
  -- 6. Trigger exists on public.farms, is BEFORE UPDATE, and is enabled.
  select
    count(*) filter (where true),
    count(*) filter (where t.tgenabled = 'D')
    into v_trig_count, v_trig_disabled
  from pg_trigger t
  ...
  raise notice 'VERIFY A PASSED: object state correct (SECURITY DEFINER, fixed search_path, is_ddp_admin, all 7 columns preserved, trigger enabled, not directly executable).';
end $$;
...
--   farm    000fa000-0000-0000-0000-000000000001
--   other   000fa000-0000-0000-0000-000000000002
...
begin
  if not exists (select 1 from pg_policies where schemaname='public' and tablename='farms'
    ...
    raise exception 'VERIFY B PRECONDITION FAILED: policy "farms: farmer update own" is absent — farmer path not reachable';
  end if;
  if not exists (select 1 from pg_policies where schemaname='public' and tablename='farms'
  ...
  if exists (select 1 from public.farms where id in
        ('000fa000-0000-0000-0000-000000000001','000fa000-0000-0000-0000-000000000002')) then
...
-- Seed fixture with FK/trigger enforcement relaxed (superuser only). This skips
-- the profiles/farms/farm_memberships -> auth.users FKs so no auth.users rows are
-- needed. The BEFORE UPDATE guard does not fire on INSERT, but replica mode makes
-- the seed fully deterministic. Switched back to origin before any UPDATE.
...
do $$
declare v_p int; v_f int; v_m int;
begin
...
-- Re-enable normal trigger/FK behaviour so the guard fires on UPDATE.
...
-- ── B5: residue check — after ROLLBACK nothing must survive. ───────────────
-- (Runs outside the rolled-back transaction. Expected: all zero.)
select
  ...
  (select count(*) from public.farms where id in
     ('000fa000-0000-0000-0000-000000000001','000fa000-0000-0000-0000-000000000002')) as leftover_farms,
...
begin
  select
    ...
    into v;
  if v <> 0 then
    raise exception 'VERIFY B5 FAILED: % test row(s) survived rollback — residue present', v;
  end if;
  raise notice 'VERIFY B5 PASSED: no test residue after rollback.';
end $$;
